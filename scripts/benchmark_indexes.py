...
# ---------------------------------------------------------------------------
# Dataset loading via ir_datasets / Hugging Face
# ---------------------------------------------------------------------------
...
def _load_hf_dataset(
    ...
    """Load documents/queries/qrels from a Hugging Face dataset.

    Expects default split names:
      - docs
      - queries
      - qrels (optional)
    And default column names:
      - docs: doc_id, text, optional title
    ...
    try:
        ...
        raise ImportError(
            "HF dataset loading requires `datasets`. Install with `pip install datasets`."
        ) from exc
    ...
    qrel_doc_id_col = "doc_id"
    qrel_relevance_col = "relevance"

    logger.info("Loading HF docs split '%s' from %s", docs_split, dataset_name)
    docs_ds = hf_load_dataset(dataset_name, split=docs_split)
    if len(docs_ds) == 0:
        raise ValueError(f"HF docs split '{docs_split}' is empty for {dataset_name}")

    sample_doc = docs_ds[0]
    missing_doc_cols = [
        col for col in [doc_id_col, doc_text_col] if col not in sample_doc
    ]
    if missing_doc_cols:
        raise ValueError(
            f"HF docs split '{docs_split}' is missing required columns: {missing_doc_cols}"
        )

    documents = []
    for doc in tqdm(docs_ds, desc="Loading HF documents", unit="docs"):
        doc_text = str(doc[doc_text_col]).strip()
        title = doc.get(doc_title_col)
        if title:
            doc_text = f"{title}\n\n{doc_text}".strip()
        documents.append({"id": str(doc[doc_id_col]), "text": doc_text})

    logger.info("Loading HF queries split '%s' from %s", queries_split, dataset_name)
    queries_ds = hf_load_dataset(dataset_name, split=queries_split)
    if len(queries_ds) == 0:
        raise ValueError(
            f"HF queries split '{queries_split}' is empty for {dataset_name}"
        )

    sample_query = queries_ds[0]
    missing_query_cols = [
        col for col in [query_id_col, query_text_col] if col not in sample_query
    ]
    if missing_query_cols:
        raise ValueError(
            "HF queries split "
            f"'{queries_split}' is missing required columns: {missing_query_cols}"
        )

    queries = {}
    for query in tqdm(queries_ds, desc="Loading HF queries", unit="queries"):
        queries[str(query[query_id_col])] = str(query[query_text_col]).strip()

    qrels = {}
    if qrels_split and qrels_split.lower() != "none":
        try:
            logger.info("Loading HF qrels split '%s' from %s", qrels_split, dataset_name)
            qrels_ds = hf_load_dataset(dataset_name, split=qrels_split)
            if len(qrels_ds) > 0:
                sample_qrel = qrels_ds[0]
                missing_qrel_cols = [
                    col
                    for col in [qrel_query_id_col, qrel_doc_id_col, qrel_relevance_col]
                    if col not in sample_qrel
                ]
                if missing_qrel_cols:
                    raise ValueError(
                        "HF qrels split "
                        f"'{qrels_split}' is missing required columns: {missing_qrel_cols}"
                    )

                for qrel in tqdm(qrels_ds, desc="Loading HF qrels", unit="qrels"):
                    qid = str(qrel[qrel_query_id_col])
                    did = str(qrel[qrel_doc_id_col])
                    rel = int(qrel[qrel_relevance_col])
                    if qid not in qrels:
                        qrels[qid] = {}
                    qrels[qid][did] = rel
        except Exception:
            logger.warning(
                "Could not load HF qrels split '%s' for %s. Proceeding without qrels.",
                qrels_split,
                dataset_name,
            )
...
def load_dataset(
    ...
    """Load documents, queries, and qrels from ir_datasets or HF.

    Supported formats:
      - ir_datasets id (e.g. ``beir/fiqa/test``)
      - HF canonical dataset name (e.g. ``Tevatron/BrowseComp-Plus-corpus``)
    """
    ...
    try:
        ...
        logger.info(
            "Dataset '%s' not found in ir_datasets. Trying Hugging Face dataset loader.",
            dataset_id,
        )
        return _load_hf_dataset(dataset_name=dataset_id)
...
def encode_documents_sharded(
    ...
    for shard_idx in to_encode:
        ...
        if use_multi_gpu:
            ...
            if pool_factor > 1:
                encode_kwargs["pool_method"] = cfg.encode.get("pool_method", "hierarchical")
            if save_token_ids:
                encode_kwargs["return_token_ids"] = True
            encode_result = model.encode(**encode_kwargs)
            if save_token_ids:
...
def load_docids_from_shards(shard_dir: Path) -> list[str] | None:
    """Read per-shard ``doc_shard_*.docids.json`` files into one ordered list.

    Returns None if no docids files are present (caller should fall back to
    ordering inferred from the dataset).
    """
    docid_files = sorted(shard_dir.glob("doc_shard_*.docids.json"))
    if not docid_files:
        return None
    docids: list[str] = []
    for path in docid_files:
        with open(path) as f:
            shard_ids = json.load(f)
        docids.extend(str(x) for x in shard_ids)
    return docids
...
def build_index(
    ...
    shard_docids = load_docids_from_shards(shard_dir)
    if shard_docids is not None:
        if documents and len(shard_docids) != len(documents):
            logger.warning(
                "Per-shard docids count (%d) != documents count (%d); using shard docids "
                "(embeddings were encoded in a different order than the dataset).",
                len(shard_docids), len(documents),
            )
        doc_ids = shard_docids
    elif documents:
        doc_ids = [doc["id"] for doc in documents]
    else:
        raise FileNotFoundError(
            f"No doc_shard_*.docids.json files in {shard_dir} and no documents provided. "
            "Run with encode_docs first or include docids files alongside the shards."
        )
    ...
    if index_type == "warp":
        indexing_kwargs = {"nbits": nbits, "n_samples_kmeans": n_samples_kmeans}
        if use_triton is not None:
            indexing_kwargs["use_triton"] = use_triton
        indexing_config = indexes.WARPIndexingConfig(**indexing_kwargs)
        index = indexes.WARP(
        ...
        logger.info("Loading shards into memory for WARP add_documents...")
...
def benchmark_search(
    ...
    if qrels:
        ...
        logger.warning("No qrels available for this dataset. Skipping metric evaluation.")
...
def load_existing_index(
    ...
    if index_type == "warp":
        indexing_config = indexes.WARPIndexingConfig(nbits=nbits)
        index = indexes.WARP(
            index_folder=index_folder,
            index_name=index_name,
            override=False,
...
def main(cfg: DictConfig) -> None:
    ...
    for dataset_id in cfg.datasets:
        ...
        # Lazy dataset loading: skip the corpus download when every required
        # piece of data can be served from on-disk artifacts.
        shard_docids_available = (
            shard_dir.exists() and bool(next(shard_dir.glob("doc_shard_*.docids.json"), None))
        )
        needs_documents = "encode_docs" in stage_set or (
            "build_index" in stage_set and not shard_docids_available
        )
        needs_queries = "encode_queries" in stage_set or "retrieve" in stage_set

        documents: list[dict] = []
        queries: dict[str, str] = {}
        qrels: dict = {}
        query_ids: list[str] = []
        if needs_documents or needs_queries:
            documents, queries, qrels = load_dataset(dataset_id)
            query_ids = list(queries.keys())
        else:
            logger.info(
                "Skipping dataset load — all required data is on disk "
                "(stages=%s, shard_docids=%s).",
                ordered_stages, shard_docids_available,
            )
        ...
        if n_documents == 0 and shard_docids_available:
            n_documents = len(load_docids_from_shards(shard_dir) or [])
