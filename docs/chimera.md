...
`cpp/include/chimera/config.cuh` fixes two numbers into the binary:

| constant | value | meaning |
|---|---|---|
| `PADDED_DIM` | 128 | dimension after rotation; embeddings must be 64-128 |
| `Q_DOCLEN` | 32 | query tokens, exactly |

The wrapper parses these back out of the source next to the build tree, so a
rebuild with different values is picked up automatically. Queries shorter than
`Q_DOCLEN` are zero-padded — score-preserving for MaxSim, since a zero query
vector contributes 0 to every document — but longer queries **raise** rather
than truncate, because silently dropping tokens would change what the benchmark
measures. Datasets in `conf/eval/config.yaml` with `query_length > 32` (scidocs,
scifact, trec-covid, arguana, climate-fever) therefore need `Q_DOCLEN` raised
(a multiple of 16) and a rebuild.
...
| chimera (`k_full_bit=300`, upstream default) | 6.9s | 117 MB | 347 | 0.3634 | 0.2903 |
| chimera (`k_full_bit=2000`, repo default) | 6.9s | 117 MB | 123 | 0.3783 | 0.3371 |
...
comparison is at lotte/MS MARCO scale, where Chimera's compressed candidate
pipeline is meant to pay off.
